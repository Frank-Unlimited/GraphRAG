...
def safe_json_loads(value: str):
    """Parse JSON string with aggressive cleaning for malformed JSON from LLMs."""
    if not value:
        return None
    ...
    try:
        ...
        # Step 1: Remove all control characters and whitespace formatting
        cleaned = value
        # Remove actual newlines, tabs, carriage returns (keep spaces)
        cleaned = re.sub(r'[\n\r\t]', ' ', cleaned)
        # Remove other control characters
        cleaned = re.sub(r'[\x00-\x08\x0b\x0c\x0e-\x1f]', '', cleaned)
        
        # Step 2: Fix invalid escape sequences (like \( \) from LaTeX)
        # Remove backslash before any character that's not a valid JSON escape
        ...
        try:
            ...
            logging.error(f"Failed to parse JSON after cleaning. Error: {e}")
            logging.error(f"Cleaned JSON (first 500 chars): {cleaned[:500]}...")
            try:
...
class DRIFTPrimer:
    ...
    async def decompose_query(
        ...
        parsed_response = safe_json_loads(response)
